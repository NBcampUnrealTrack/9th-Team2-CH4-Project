...
protected:
    // [수정] 검색 타임아웃 및 세션 브라우저 자동 갱신 타이머와 함수
    FTimerHandle SearchTimeoutTimerHandle;
    FTimerHandle AutoRefreshSessionsTimerHandle;

    void OnSearchTimeout();
    void RequestSilentSessionRefresh();

    // [수정] GameState/PlayerState 복제 완료될 때까지 재시도하는 안전장치 타이머와 함수
    FTimerHandle StateBindRetryTimerHandle;
    void TryBindLobbyStates();
};
